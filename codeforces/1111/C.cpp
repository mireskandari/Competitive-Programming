...
ll Solve(ll l, ll r) {
    ll low = lower_bound(all(in), l) - in.begin();
    ll upper = upper_bound(all(in), r) - in.begin();
    ll sz = upper - low;
    ll fans = sz > 0 ? b * sz * (r - l + 1) : a;
    if (r - l <= 0 || sz == 0) return fans;
    ll m = (r + l) >> 1;
    return min(Solve(l, m) + Solve(m + 1, r), fans);
}
