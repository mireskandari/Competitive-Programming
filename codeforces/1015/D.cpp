...
int main() {
    ...
    cin >> n >> k >> s;
    ll maxd = n - 1;
    if (ceil((ld)s / k) > maxd || k > s) return cout << "NO", 0;
    cout << "YES\n";
    ll each = s / k;
    ll divv = s % k;
    vector<ll> diff(k, each);
    for (int i = 0; divv > 0; i++, divv--) {
        diff[i] += 1;
    }
    vector<ll> movee(k + 1, 0);
    movee[0] = 1;
    rep (1, k + 1, i) {
        if (movee[i - 1] + diff[i - 1] <= n) {
            movee[i] = movee[i - 1] + diff[i - 1];
        } else {
            movee[i] = movee[i - 1] -  diff[i - 1];
        }
        cout << movee[i] << ' ';
